...
public class TelegramClientImpl implements TelegramClient {
	...
	private static final Logger log = Loggers.getLogger(TelegramClientImpl.class);
	...
	@Inject
	public TelegramClientImpl(SecretsManager secretsManager, final NewMessageHandler newMessageHandler) {
...
//        Hooks.onOperatorDebug();
		...
		MTProtoTelegramClient.create(secretsManager.telegram().apiId(), secretsManager.telegram().apiHash(),
				// Linux way
				Boolean.getBoolean("useQrAuth") ? new QRAuthorizationHandler(new QRCallback())
						: new CodeAuthorizationHandler(new StdINCallback()))
				.setEntityRetrieverStrategy(
						EntityRetrievalStrategy.preferred(EntityRetrievalStrategy.STORE_FALLBACK_RPC,
								PreferredEntityRetriever.Setting.FULL, PreferredEntityRetriever.Setting.FULL))
...
//                        Path.of("C:\\work\\t4j.bin")))
				...
				.setUpdatesManager(client -> new DefaultUpdatesManager(client,
						new Options(Options.DEFAULT_CHECKIN, Options.MAX_USER_CHANNEL_DIFFERENCE, true)))
				.withConnection(client -> {

					Mono<Void> eventLog = client.getMtProtoClientGroup().updates().on(BaseUpdates.class)
							.doOnNext(newMessageHandler::handle).then();
						...
						var task = scheduler.schedule(() -> {
							while (!Thread.currentThread().isInterrupted()) {
								try {
									...
									log.info("Delaying {} status for {} seconds", online[0] ? "offline" : "online",
											sleep);
					...
					}).takeUntilOther(client.onDisconnect()).flatMap(e -> {
	...
	static class QRCallback extends Base2FACallback implements QRAuthorizationHandler.Callback {
		...
		static String generateQr(String text) {
			try {
				...
				return v.inputReader(StandardCharsets.UTF_8).lines().collect(Collectors.joining("\n"));
			} catch (IOException e) {
